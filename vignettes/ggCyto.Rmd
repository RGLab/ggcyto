...
ggplot(fs[1], aes(x = `FL1-H`)) + geom_histogram()

# customize border colors and log scale 
ggplot(fs[1], aes(x = `FL1-H`)) + geom_histogram(colour = "white") + scale_x_log10()

# flowJo biexponential scale (shorthand: with default parameters setting e.g.maxValue = 262144)
ggplot(fs[1], aes(x = `FL1-H`)) + geom_histogram(colour = "white") + scale_x_flowJo_biexp() 

# flowJo biexponential scale (longer form: with customized parameters)
ggplot(fs[1], aes(x = `FL1-H`)) + geom_histogram(colour = "white")  + scale_x_flowJo_biexp(maxValue = 1e4)

# myBreaks <- c(0, 10, 1e2, 1e3)
ggplot(fs[1], aes(x = `FL1-H`)) + geom_histogram(colour = "white")  + scale_x_flowJo_biexp(maxValue = 1e4, breaks = c(0, 10, 100, 1000))
