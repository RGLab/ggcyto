...
# add contour
p1 + geom_density2d(colour = "black")
...
ggplot(fs, aes(x = `FSC-H`, y =  `SSC-H`)) + 
          ...
          geom_point(data = overlay.fr, color = "red", cex = 0.5, pch = 19) +
          geom_point(data = overlay.fr2, color = "black", cex = 0.5, pch = 19) +
...
# 2d rectangleGate
p + geom_gate(data = rect.g)
# 1d rectangleGate
rect.g1d <- rectangleGate("FSC-H" =  c(550, Inf))
p + geom_gate(data = rect.g1d, aes(x = `FSC-H`, y =  `SSC-H`))
