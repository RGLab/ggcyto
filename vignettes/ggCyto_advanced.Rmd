...
# pData must be explicitly passed to geom_gate layer for facetting
p + geom_gate(data = poly.gates, pd = pData(fs))

# original mapping must be provided in order for 1d gate to determine between hline vs vline
orig_aes <- aes(x = `FSC-H`, y =  `SSC-H`)
den.gates <- fsApply(fs, openCyto::mindensity, channel = "FSC-H", gate_range = c(100, 300))
p + geom_gate(data = den.gates, pd = pData(fs), mapping = orig_aes)

den.gates <- fsApply(fs, openCyto::mindensity, channel = "SSC-H", gate_range = c(100, 500))
p + geom_gate(data = den.gates, pd = pData(fs), mapping = orig_aes)
