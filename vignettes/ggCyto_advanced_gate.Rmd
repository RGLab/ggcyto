---
title: "Visualize cytometry data with ggplot2 (advanced) -- Gate"
author: "Mike Jiang"
date: "04/13/2015"
...
centroids <- ldply(poly.gates, function(gate){
                          df <- fortify(gate)
                          colMeans(df)
                          }, .id = ".rownames")
df <- merge(centroids, stats)
df <- merge(df, name_rows(pData(fs)))
p + geom_gate(data = poly.gates, pd = pData(fs)) + geom_btext(data = df , aes(label = percent), size = 3)
...
my_aes <- aes(x = `SSC-H`)
ggplot(fs, my_aes) + geom_density() + facet_wrap(~name) + geom_gate(den.gates, pd = pData(fs))
```
