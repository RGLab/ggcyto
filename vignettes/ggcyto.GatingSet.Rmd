...
# 2d plot
p <- p + geom_hex(bins = 32, na.rm = T)
p

# raw scale + limits
p <- p + scale_x_raw(limits = c(0, 4e3)) + scale_y_raw(limits = c(0, 4e3))
p
...
# change type
p1 + geom_stats("4+", type = "count")
```

```{r error=T}
# 'subset' is abstract without specifiying it
p <- ggcyto(gs, aes(x = CD4, y = CD8)) + geom_hex() + scale_x_raw(limits = c(0, 4e3)) + scale_y_raw(limits = c(0, 4e3))
p
```
...
p <- ggcyto(gs, aes(x = IFNg, y = IL2), subset = "4+") + geom_hex() + scale_x_raw(limits = c(0, 4e3)) + scale_y_raw(limits = c(0, 4e3))
p

p + geom_gate() + geom_stats(fill = "yellow")

# add gates to the arbitary(non-parent) node
p2 <- ggcyto(gs, subset = "3+", aes(x = IFNg, y = IL2)) + geom_hex() + scale_x_raw(limits = c(0, 4e3)) + scale_y_raw(limits = c(0, 4e3))
